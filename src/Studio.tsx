    ...
    <div className="studio-page">
      <div className="studio-header">
        <div>
          ...
          <h1>{admin ? '슈퍼관리자' : '크리에이터 스튜디오'}</h1>
          <p>{user.name}님, 오늘도 좋은 이야기를 만들어 주세요.</p>
        </div>
      ...
      <div className="studio-tabs">
        {[
          ...
          { id: 'contents', name: admin ? '콘텐츠 심사' : '내 작품', icon: Film },
          { id: 'revenue', name: '매출 · 정산', icon: Wallet },
          ...(admin ? [{ id: 'users', name: '회원 관리', icon: Users }] : []),
      ...
      {tab === 'overview' && (
        <>
          ...
          <div className="studio-callout">
            ...
            <button
              className="primary"
              onClick={() => (admin ? setTab('contents') : setEditor('new'))}
            >
              {admin ? '심사하기' : '작품 등록'}
              <Plus size={16} />
            </button>
      ...
      {tab === 'contents' && (
        <>
          <div className="section-heading">
            <h2>
              {admin ? '전체 콘텐츠' : '내 작품'} <span className="lime">{data.dramas.length}</span>
            </h2>
          ...
          <div className="studio-filters">
            {[
              ...
              <button className={filter === v ? 'active' : ''} key={v} onClick={() => setFilter(v)}>
                {t}
              </button>
            ))}
          </div>
          ...
          {!data.dramas.some((d) => filter === 'all' || d.status === filter) && (
          ...
          {data.users.map((u) => (
            <div className="user-row" key={u.id}>
              ...
              <div>
                ...
                <span>{u.email}</span>
                <small>
                  {u.role === 'admin' ? '슈퍼관리자' : u.role === 'pd' ? '업로더 (PD)' : '시청자'} ·{' '}
