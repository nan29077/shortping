...
export default function PreviewPlayer({
  ...
  const playable = useMemo(() => shots.filter((s) => s.video || s.image), [shots]);
      ...
      <div className="preview-frame">
        <div className="preview-screen">
          {shot.video ? (
            ...
            <img key={shot.id} className="kenburns" src={shot.image} alt="" style={{ animationDuration: `${durations[index]}s` }} />
          )}
          {shot.audio && <audio key={'a' + shot.id} ref={audio} src={studioMedia(shot.audio)} preload="auto" />}
          {shot.dialogue && (
            <p className="preview-subtitle">
              {speaker ? <b>{speaker.name}</b> : null}
              {shot.dialogue}
            </p>
          )}
          <span className="preview-tag">
            #{index + 1} · {shot.camera || '컷'} {shot.video ? '· 영상' : '· 스토리보드'}
          </span>
