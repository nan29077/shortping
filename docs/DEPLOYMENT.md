...
홈과 공개 작품·방송국의 공유 주소에는 카카오톡 등에서 읽는 Open Graph 제목·설명·이미지가 서버에서 제공됩니다. 운영 환경의 `APP_ORIGIN`은 외부에서 접근 가능한 실제 HTTPS 도메인으로 지정하고, `/images/share-shortping.jpg`가 로그인 없이 열리는지 확인하세요. `localhost` 링크는 다른 사람의 기기나 카카오 서버에서 접근할 수 없습니다. 작품·방송국 공유 버튼은 각각 `/share/drama/{id}`, `/share/channel/{id}` 주소를 사용하며, 방문자는 기존 웹앱의 해당 화면으로 이동합니다. 비공개 작품과 방송국은 공유 카드가 나오지 않습니다.
